...
#include "SymmetryFunctionAngularWideShift.h"
#include "Atom.h"
...
SymmetryFunctionAngularWideShift::
SymmetryFunctionAngularWideShift(ElementMap const& elementMap) :
    SymmetryFunction(91, elementMap),
    useIntegerPow(false),
    e1           (0    ),
    e2           (0    ),
    zetaInt      (0    ),
    ...
    rs           (0.0  )
{
    ...
    parameters.insert("thetaShift");
    parameters.insert("rs");
}

bool SymmetryFunctionAngularWideShift::
operator==(SymmetryFunction const& rhs) const
{
    if (ec   != rhs.getEc()  ) return false;
    if (type != rhs.getType()) return false;
    SymmetryFunctionAngularWideShift const& c =
        dynamic_cast<SymmetryFunctionAngularWideShift const&>(rhs);
    ...
    if (thetaShift  != c.thetaShift ) return false;
...
operator<(SymmetryFunction const& rhs) const
{
    ...
    if      (thetaShift  < c.thetaShift ) return true;
    else if (thetaShift  > c.thetaShift ) return false;
...
void SymmetryFunctionAngularWideShift::
setParameters(string const& parameterString)
{
    ...
    thetaShift = atof(splitLine.at(5).c_str());
...
void SymmetryFunctionAngularWideShift::changeLengthUnit(double convLength)
{
...
string SymmetryFunctionAngularWideShift::getSettingsLine() const
{
...
void SymmetryFunctionAngularWideShift::calculate(Atom&      atom,
                                            bool const derivatives) const
{
    double const pnorm  = pow(2.0, 1.0 - zeta);
    double const pzl    = zeta;// * lambda; // TODO!!!
    double       result = 0.0;
                    ...
                    (e2 == nej && e1 == nek))
                {
                    ...
                    // Energy calculation.
                        ...
                        double const pexp = exp(-eta * (rijs * rijs
                                                      + riks * riks));
                        double const plambda = 1.0 + 1.0 * costijk; // TODO!!
                        double       fg = pexp;
...
string SymmetryFunctionAngularWideShift::parameterLine() const
{
...
vector<string> SymmetryFunctionAngularWideShift::parameterInfo() const
{
    ...
    v.push_back(strpr((pad(s, w) + "%14.8E").c_str(),
                      eta * convLength * convLength));
    s = "thetaShift";
    v.push_back(strpr((pad(s, w) + "%14.8E").c_str(), thetaShift));
    s = "zeta";
...
double
SymmetryFunctionAngularWideShift::calculateRadialPart(double distance) const
{
...
double
SymmetryFunctionAngularWideShift::calculateAngularPart(double angle) const
{
    return 2.0 * pow((1.0 + cos(angle - M_PI / 180.0 * thetaShift)) / 2.0,
                     zeta);
}
