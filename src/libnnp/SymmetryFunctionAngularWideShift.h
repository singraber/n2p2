...
#ifndef SYMMETRYFUNCTIONANGULARWIDESHIFT_H
#define SYMMETRYFUNCTIONANGULARWIDESHIFT_H
...
/** Angular symmetry function with shift (type 91)
 *
 * @f[
   G^{91}_i = 2^{1-\zeta} \sum_{\substack{j,k\neq i \\ j < k}}
           \left( 1 + \cos (\theta_{ijk} -\theta_s) \right)^\zeta
           \mathrm{e}^{-\eta( (r_{ij}-r_s)^2 + (r_{ik}-r_s)^2 ) }
           f_c(r_{ij}) f_c(r_{ik}) 
 * @f]
 * Parameter string:
 * ```
 * <element-central> 91 <element-neighbor1> <element-neighbor2> <eta> <thetashift> <zeta> <rcutoff> <<rshift>>
 * ```
 * where
 ...
 * - `<eta> .................` @f$\eta@f$ 
 * - `<thetashift> ..........` @f$\theta_s@f$ 
 * - `<zeta> ................` @f$\zeta@f$ 
 * - `<rcutoff> .............` @f$r_c@f$
 * - `<<rshift>> ............` @f$r_s@f$ (optional, default @f$r_s = 0@f$)
 */
class SymmetryFunctionAngularWideShift : public SymmetryFunction
{
public:
    /** Constructor, sets type = 91
     */
    ...
    /** Get private #thetaShift member variable.
     */
    double       getThetaShift() const;
...
private:
    ...
    int         zetaInt;
    /// Angular shift value @f$\theta_s@f$.
    double      thetaShift;
...
inline bool SymmetryFunctionAngularWideShift::
...
inline bool SymmetryFunctionAngularWideShift::getUseIntegerPow() const
{
    return useIntegerPow;
}

inline std::size_t SymmetryFunctionAngularWideShift::getE1() const
{
    return e1;
}

inline std::size_t SymmetryFunctionAngularWideShift::getE2() const
{
    return e2;
}

inline int SymmetryFunctionAngularWideShift::getZetaInt() const
{
    return zetaInt;
}

inline double SymmetryFunctionAngularWideShift::getThetaShift() const
{
    return thetaShift;
}

inline double SymmetryFunctionAngularWideShift::getEta() const
{
    return eta;
}

inline double SymmetryFunctionAngularWideShift::getZeta() const
{
    return zeta;
}

inline double SymmetryFunctionAngularWideShift::getRs() const
{
    return rs;
}
